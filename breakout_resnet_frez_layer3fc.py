...
class ResNet_DQN(nn.Module): ##  num_clss to action_size;
    ...
    def __init__(self, block, layers, action_size=4, in_channels=4):
        self.inplanes = 64
...
def resnet18_DQN(pretrained=False, action_size=4, in_channels=4, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_DQN(BasicBlock, [2, 2, 2, 2], action_size=4, in_channels=4, **kwargs)
    return model
...
for param in agent.model.layer3.parameters():
    param.requires_grad = True
agent.model.fc = nn.Linear(256,4)
agent.model=agent.model.to(device)
agent.update_target_model()
agent.optimizer = optim.Adam([{'params': agent.model.layer3.parameters()},
                              {'params': agent.model.fc.parameters()}], lr=0.00025)
#agent.epsilon = 0.04999683333377103
#agent.train_start = 10000
'''
num_ftrs = agent.model.fc.in_features
agent.model.fc=nn.Linear(num_ftrs, action_size)
agent.model=agent.model.to(device)
for param in agent.model.parameters():
    param.requires_grad = False
for param in agent.model.layer3.parameters():
    param.requires_grad = True
for param in agent.model.fc.parameters():
    param.requires_grad = True
'''
...
#         list(agent.model.fc.parameters())
        ...
        agent.append_sample(state, action, reward+done*(-5), next_state, done)
